...
> Документ описывает **код, который есть**, а не план. Ориентир при разработке —
> `TELEGRAM_LOGIN_MECHANICS.md` (механика прежней платформы Grammerce); отличия
> перечислены в §7 и сделаны осознанно.
...
| Бот @financePro | контейнер `bot`, [server/app/bot.py](server/app/bot.py) | точка входа с компьютера: на `/start` присылает одноразовую ссылку |
...
Отдельного «шоп-бота» и второго токена, как в Grammerce, здесь нет: бот один.
...
- Поиск пользователя — только по id провайдера. Синтетических email нет вовсе, поэтому
  атака «зарегистрируй `tg_<чужой_id>@…`» из §6 документа Grammerce здесь невозможна.
...
## 7. Осознанные отличия от Grammerce

| Grammerce | finlo | Почему |
|---|---|---|
| `X-Bot-Secret` + `PLATFORM_BOT_SHARED_SECRET` | короткий JWT `{"bot": true}` на `JWT_SECRET` | секрет у обоих контейнеров уже общий, лишней переменной не нужно |
| Bearer в query: `/login?token=…` | во фрагменте: `/login#token=…` | не попадает в логи nginx и в `Referer` |
| Opaque-токен + Redis, sliding TTL | JWT, состояние не хранится | одна SQLite-база, отдельный Redis не нужен |
| Второй фактор для суперадмина | нет | админ пока только читает список пользователей; вынесено в отдельную задачу |
| `/setdomain` обязателен | не нужен | Login Widget не используется |
